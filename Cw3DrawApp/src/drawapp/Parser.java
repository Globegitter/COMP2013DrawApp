...
public class Parser {
	...
	private void drawRect(String args) throws ParseException {
		...
		if ((x1 < 0) || (x2 < 0) || (y1 < 0) || (y2 < 0))
			throw new ParseException(
					"Invalid values for the draw rectangles command.");
	...
	private void fillRect(String args) throws ParseException {
		...
		if ((x1 < 0) || (x2 < 0) || (y1 < 0) || (y2 < 0))
			throw new ParseException(
					"Invalid values for the draw fill rectangle command.");
	...
	private void drawOval(String args) throws ParseException {
		...
		if ((x1 < 0) || (y1 < 0) || (width < 0) || (height < 0))
			throw new ParseException(
					"Invalid values for the draw oval command.");
	...
	private void drawString(String args) throws ParseException {
		...
		if ((x < 0) || (y < 0))
			throw new ParseException(
					"Invalid values for the draw string command.");
	...
	private void drawImage(String args) throws ParseException {
		...
		if ((x < 0) || (y < 0) || (width < 0) || (height < 0))
			throw new ParseException(
					"Invalid values for the draw image command.");
	...
	public void parseButton(final Button nextStep, final Button complete)
			throws IOException {
		String line = reader.readLine();
		final ArrayList<String> storeCommads = new ArrayList<>();
		while (line != null) {
		...
		nextStep.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event) {
				try {
					parseLine(storeCommads.get(commandNr));
					commandNr++;
					frame.postMessage("Next Step completed.");
					if (commandNr == storeCommads.size()) {
		...
		complete.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event) {
				try {
					while (commandNr < storeCommads.size()) {
						parseLine(storeCommads.get(commandNr));
						commandNr++;
						if (commandNr == storeCommads.size()) {
							complete.setDisable(true);
